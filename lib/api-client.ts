...
export class OpenDataApiClient {
  ...
  async getSupportAndCarePullRequests() {
    ...
    return Promise.all([prsPromise, projectsPromise]).then(([prs, projects]) => {
      const filteredProjects = projects.filter((project: { uuid: string; matchingRepos?: string[] }) =>
        SPECIAL_PROJECT_UUIDS.includes(project.uuid),
      )
      const relevantPatterns = filteredProjects.flatMap(
        (project: { uuid: string; matchingRepos?: string[] }) => project.matchingRepos || [],
      )
